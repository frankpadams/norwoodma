...
function text(b){return [b.name,b.category,b.address,...(b.labels||[]),...(b.tags||[])].join(' ').toLowerCase();}
...
function card(b){const reviews=review(b.googleReviews,'Google reviews','google.com')+review(b.yelp,'Yelp reviews','yelp.com')+review(b.tripadvisor,'Tripadvisor reviews','tripadvisor.com');const labels=(b.labels&&b.labels.length?b.labels:[b.category]).filter(Boolean);return '<article class="business-card"><div><span class="badge">'+esc(labels.join(' · '))+'</span><h3>'+esc(b.name)+'</h3><p>'+esc(b.address)+'</p>'+(b.phone?'<p><a href="tel:'+b.phone.replace(/[^0-9+]/g,'')+'">'+esc(b.phone)+'</a></p>':'')+'</div><div class="business-actions">'+(b.website?'<a class="business-website" href="'+esc(b.website)+'" target="_blank" rel="noopener">Website ↗</a>':'')+(reviews?'<span class="review-icons" aria-label="Verified review-site listings">'+reviews+'</span>':'')+'</div></article>'}
...
function render(){
 ...
 else if(selected) rows=all.filter(b=>(b.labels||[b.category]).includes(selected)||b.groups.includes(selected));
...
function init(){
 syncFromUrl();
 const cats=[...new Set(all.flatMap(b=>b.labels||[b.category]).filter(Boolean))].sort();
