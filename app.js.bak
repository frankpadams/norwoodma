...
function eventSummary(e){const bits=[];if(e.start?.time)bits.push(formatEventTime(e.start.time));if(e.end?.date&&e.end.date!==e.start?.date)bits.push(`${shortDate(e.start.date)}–${shortDate(e.end.date)}`);if(e.venue)bits.push(e.venue);if(e.cost)bits.push(e.cost);return bits.join(' · ')||e.address||'Open source for details.';}
