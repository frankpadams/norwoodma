...
function renderEventsPage(items){
 ...
 if(eventId){
   ...
   if(e){
     ...
     const links=[];const addLink=(url,label)=>{if(url&&!links.some(x=>x.url===url))links.push({url,label})};addLink(e.registration_url,'Participate / register');addLink(e.donation_url,'Donate / support');addLink(e.purchase_url,'Purchase / order');addLink(e.source_url,'Official event information');(Array.isArray(e.links)?e.links:[]).forEach(x=>{if(typeof x==='string')addLink(x,'More information');else if(x&&x.url)addLink(x.url,x.label||'More information')});const linkHtml=links.length?'<div class="event-detail-links"><strong>Links:</strong>'+links.map(x=>'<p><a href="'+esc(x.url)+'" target="_blank" rel="noopener">'+esc(x.label)+' ↗</a></p>').join('')+'</div>':'';
